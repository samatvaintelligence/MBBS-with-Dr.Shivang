...
// UPI payment — paste Dr. Shivang's UPI ID here (e.g. "shivang@sbi")
const UPI_ID = "";
const UPI_NAME = "Dr Shivang Gupta";
const BOOKING_FEE_AMOUNT = 15000;
const BOOKING_FEE_NOTE = "MBBS Booking Fee";
...
function getUpiPayUrl() {
  return `upi://pay?pa=${encodeURIComponent(UPI_ID)}&pn=${encodeURIComponent(UPI_NAME)}&am=${BOOKING_FEE_AMOUNT}&cu=INR&tn=${encodeURIComponent(BOOKING_FEE_NOTE)}`;
}

function getUpiQrSrc() {
  return `https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(getUpiPayUrl())}&size=220x220&margin=8`;
}
...
function initAdmissionsForm() {
  ...
  function showPaymentSection() {
    ...
    const statsGrid = document.getElementById("form-stats");
    ...
    if (UPI_ID) {
      const qrImg = paymentSection.querySelector("[data-upi-qr]");
      const upiBtn = paymentSection.querySelector("[data-upi-pay]");
      if (qrImg) qrImg.src = getUpiQrSrc();
      if (upiBtn) upiBtn.href = getUpiPayUrl();
    } else {
      const upiBlock = paymentSection.querySelector("[data-upi-block]");
      const upiBtn = paymentSection.querySelector("[data-upi-pay]");
      if (upiBlock) upiBlock.classList.add("hidden");
      if (upiBtn) upiBtn.classList.add("hidden");
    }

    const waBtn = paymentSection.querySelector("[data-wa-payment]");
    if (waBtn) {
      const msg = `I've paid the ₹15,000 booking fee for MBBS 2026. Please confirm my seat. — ${state.fullName}`;
